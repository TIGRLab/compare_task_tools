...
nistats_mat_list <- list.files(path= "/projects/ttan/fMRI_tools/compare_task_tools/nistats",recursive=T,full.names=T, 
                               pattern="^sub.*emp_combined\\.tsv$")
...
nistats_mat <- lapply(nistats_mat_list, read.table, header=F, skip=1, col.names=c("skip","ea_block","ea_press","circles",
                    "circ_press","ea_pmod",rep("confounds",73)))

# add subject ids
names(spm_mat) <- substring(spm_mat_list,1,11)
names(afni_mat) <- substring(afni_mat_list,1,11)
names(nistats_mat) <- substring(nistats_mat_list,54,64)

# collapse values across runs (SPM and nistats) and remove unwanted columns
...
for (i in names(nistats_mat)) {
  nistats_mat[[i]] <- nistats_mat[[i]][,2:79]
